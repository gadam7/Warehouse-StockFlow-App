...
import com.adamidis.learning.warehousestockflow.Event.NewUserEvent;
import com.adamidis.learning.warehousestockflow.Exception.ApiException;
import com.adamidis.learning.warehousestockflow.Form.*;
...
import com.adamidis.learning.warehousestockflow.Provider.TokenProvider;
import com.adamidis.learning.warehousestockflow.Service.EventService;
...
import static com.adamidis.learning.warehousestockflow.DTO.UserDTOMapper.toUser;
import static com.adamidis.learning.warehousestockflow.Enum.EventType.*;
import static com.adamidis.learning.warehousestockflow.Utils.ExceptionUtils.processError;
import static com.adamidis.learning.warehousestockflow.Utils.UserUtils.getAuthenticatedUser;
import static com.adamidis.learning.warehousestockflow.Utils.UserUtils.getLoggedInUser;
...
public class UserController {
    ...
    public ResponseEntity<HttpResponse> login(@RequestBody @Valid LoginForm loginForm) {
        System.out.println("Attempting login for user: " + loginForm.getEmail());
    ...
    public ResponseEntity<HttpResponse> saveUser(@RequestBody @Valid User user) {
        ...
        return ResponseEntity.created(getUri()).body(
                HttpResponse.builder()
                        ...
                        .message(String.format("User account created successfully for user %s", user.getFirstName()))
                        .status(CREATED)
    ...
    public ResponseEntity<HttpResponse> resetPassword(@PathVariable("email") String email) {
        ...
        return ResponseEntity.ok().body(
                HttpResponse.builder()
                        .timeStamp(now().toString())
                        .message("Email sent to " + email + " with instructions to reset password.")
                        .status(OK)
                        .statusCode(OK.value())
                        .build()
        );
    ...
    public ResponseEntity<HttpResponse> verifyAccount(@PathVariable("key") String key) {
        return ResponseEntity.ok().body(
                HttpResponse.builder()
                        .timeStamp(now().toString())
                        .message(userService.verifyAccountKey(key).isEnabled() ? "Account already verified" : "Account verification successful.")
                        .status(OK)
                        .statusCode(OK.value())
                        .build()
        );
    ...
    private ResponseEntity<HttpResponse> sendVerificationCode(UserDTO user) {
        ...
        return ResponseEntity.ok().body(
                HttpResponse.builder()
                        .timeStamp(now().toString())
                        .data(of("user", user))
                        .message("Verification code sent to registered email. Please verify to complete login.")
                        .status(OK)
                        .statusCode(OK.value())
                        .build()
        );
